...
describe "Schedule model object" do
  ...
  it 'should create new schedule' do
    expect(schedule1).to_not be(nil)
  end

  it 'should belong to student' do
    create_schedules
    expect(StudentSchedule.find_by({schedule_id: schedule1.id, student_id: userJohn.id })).not_to eq(nil)
  end

  it 'should belong to group' do
    create_schedules
    expect(StudentSchedule.find_by({schedule_id: schedule1.id, group_id: group1.id })).not_to eq(nil)
  end
